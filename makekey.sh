...
#利用私钥生成CSR
openssl req -days 122 -new -key output/client-key.pem -config ./_cfg/openssl.cnf -out output/./output/client-csr.pem

#生成客户端证书
openssl x509 -req -days 122 -CA output/ca-cert.pem -CAkey output/ca-key.pem -CAcreateserial -in output/client-csr.pem -out output/./output/client-cert.pem
