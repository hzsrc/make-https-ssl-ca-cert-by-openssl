...
#通过CA私钥生成CSR
openssl req -batch -passin "pass:${pwd}" -new -days 122 -key output/ca-key.pem -config _cfg/openssl.cnf -out output/ca-csr.pem
...
#--创建服务器端证书
#为服务器生成私钥
openssl genrsa -passout "pass:${pwd}" -out output/server-key.pem 2048

#利用服务器私钥文件服务器生成CSR
openssl req -batch -passin "pass:${pwd}" -new -days 122 -key output/server-key.pem -config _cfg/openssl.cnf -out output/server-csr.pem
...
#利用私钥生成CSR
openssl req -batch -passin "pass:${pwd}" -days 122 -new -key output/client-key.pem -config _cfg/openssl.cnf -out output/client-csr.pem
